...
class SccSmoother:
  ...
  def cal_max_speed(self, frame, CC, CS, sm, clu11_speed, controls):
    ...
    apply_limit_speed, road_limit_speed, left_dist, first_started, max_speed_log = \
      road_speed_limiter_get_max_speed(cluster_speed, controls.v_cruise_kph)
  ...
  def update(self, enabled, can_sends, packer, CC, CS, frame, apply_accel, controls):
    ...
    if self.wait_timer > 0:
      ...
      if self.alive_timer == 0:
        self.btn = self.get_button(clu11_speed, CS.cruiseState_speed * self.speed_conv_to_clu)
  ...
  def cal_target_speed(self, accel, CC, CS, clu11_speed, controls):
    ...
    if not self.longcontrol:
      if CS.gas_pressed:
        ...
        if clu11_speed + SYNC_MARGIN > controls.v_cruise_kph and self.sync_set_speed_while_gas_pressed:
          set_speed = clip(clu11_speed + SYNC_MARGIN, MIN_SET_SPEED_KPH, MAX_SET_SPEED_KPH)
      ...
      if self.max_speed_clu > self.min_set_speed_clu:
        self.target_speed = clip(self.target_speed, MIN_SET_SPEED_KPH, MAX_SET_SPEED_KPH)
